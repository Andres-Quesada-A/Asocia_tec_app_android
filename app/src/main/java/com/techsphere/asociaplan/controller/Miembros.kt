...
suspend fun getAllEstudiantesMiembrosBD(Id:Int) : MutableList<Miembro>{
    var conn : Connection? = null
    try {
        Class.forName("net.sourceforge.jtds.jdbc.Driver")
        conn = DriverManager.getConnection(connectionString)
        var cs = conn.prepareCall("{call BuscarMiembroAsociacion @inIdAsociacion=?, @inNombre=?, @outCodeResult=?}")
        // Asumimos que se nos pasan valores no nulos
        cs.setInt(1, Id)
        cs.setNull(2, Types.VARCHAR)
        cs.registerOutParameter(3, Types.INTEGER)
...
suspend fun getEstudiantesMiembrosBusqueda(Id:Int, Nombre: String) : MutableList<Miembro>{
    var conn : Connection? = null
    try {
        Class.forName("net.sourceforge.jtds.jdbc.Driver")
        conn = DriverManager.getConnection(connectionString)
